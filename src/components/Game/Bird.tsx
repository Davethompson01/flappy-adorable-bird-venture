...
const Bird: React.FC<BirdProps> = ({
  ...
  useEffect(() => {
    if (birdRef.current) {
      birdRef.current.style.transition = isDead
        ? "none"
        : "transform 0.1s ease";
    }
  }, [isDead]);
    ...
    >
      <div
        className={`w-full h-full relative ${
          isFlapping && !isDead ? "animate-[pulse_0.4s_ease-in-out]" : ""
        }`}
      >
        <img
          ...
          className={`w-full h-full object-contain ${
            isDead ? "opacity-75" : "opacity-100"
          } transition-all duration-300`}
          style={{
            filter: isDead ? "grayscale(50%)" : "none",
            transform:
              isFlapping && !isDead ? "translateY(-3px)" : "translateY(0)",
